...
function ajaxpage(url, containerid,ret){
	...
	finalRet=ret;
start="SUB_Physics";
url=url+'?selection='+start+','+ret+'&question=0';
alert(url);
...
function fnGetNextQn(location){
		...
		alert(question);
		start="SUB_Physics"
		url=finalURL+'?selection='+start+','+finalRet+'&question='+question;
		var page_request = false
		if (window.XMLHttpRequest) // if Mozilla, Safari etc
		page_request = new XMLHttpRequest()
		else if (window.ActiveXObject){ // if IE
	...
	border: thin solid 1 1 1 1;
		...
		dtmlXMLLoaderObject.prototype.getXMLTopNode = function(tagName) {
			if (this.xmlDoc.responseXML) {
				var temp = this.xmlDoc.responseXML
		...
		dtmlXMLLoaderObject.prototype.loadXML = function(filePath) {
			try {
				...
				if (document.implementation
					...
					this.xmlDoc.onreadystatechange = new this.waitLoadFunction(
							this);
				}
		...
		dhtmlDragAndDropObject.prototype.removeDraggableItem = function(
				htmlNode) {
		...
		dhtmlDragAndDropObject.prototype.stopDrag = function(e) {
			...
			if (dragger.lastLanding)
				dragger.lastLanding.dragLanding._drag(dragger.dragStartNode,
						dragger.dragStartObject, dragger.lastLanding);
		...
		function dhtmlXTreeObject(htmlObject, width, height, rootId) {
			...
			this.lineArray = new Array("line2.gif", "line3.gif", "line4.gif",
					"blank.gif", "blank.gif");
			this.minusArray = new Array("minus2.gif", "minus3.gif",
					"minus4.gif", "minus.gif", "minus5.gif");
		...
		function dhtmlXTreeItemObject(itemId, itemText, parentObject,
				treeObject, actionHandler, mode) {
			this.htmlNode = "";
			this.acolor = "";
			this.scolor = "";
			this.tr = 0;
			...
			this.images = new Array(treeObject.imageArray[0],
					treeObject.imageArray[1], treeObject.imageArray[2]);
		...
		dhtmlXTreeObject.prototype._globalIdStorageAdd = function(itemId,
				itemObject) {
		...
		dhtmlXTreeObject.prototype._attachChildNode = function(parentObject,
				itemId, itemText, itemActionHandler, image1, image2, image3,
				optionStr, childs, beforeNode) {
			if (beforeNode)
				parentObject = beforeNode.parentObject;
			if (((parentObject.XMLload == 0) && (this.XMLsource))
					&& (!this.XMLloadingWarning)) {
				parentObject.XMLload = 1;
				this.loadXML(this.XMLsource + getUrlSymbol(this.XMLsource)
						+ "itemId=" + escape(parentObject.itemId));
			}
			...
			Nodes[Count] = new dhtmlXTreeItemObject(itemId, itemText,
		...
		dhtmlXTreeObject.prototype.insertNewItem = function(parentId, itemId,
				itemText, itemActionHandler, image1, image2, image3, optionStr,
				childs) {
			var parentObject = this._globalIdStorageFind(parentId);
			if (!parentObject)
				return (-1);
			return this._attachChildNode(parentObject, itemId, itemText,
					itemActionHandler, image1, image2, image3, optionStr,
					childs);
		};
		...
		dhtmlXTreeObject.prototype._parseXMLTree = function(dhtmlObject, node,
			...
			for ( var i = 0; i < node.childNodes.length; i++) {
				if ((node.childNodes[i].nodeType == 1)
						&& (node.childNodes[i].tagName == "item")) {
					...
					var closeable = node.childNodes[i]
							.getAttribute("closeable");
					...
					temp.XMLload = 1;
					dhtmlObject.insertNewItem(parentId, cId, name, 0, im0, im1,
							im2, zST, chd);
					if (dhtmlObject.parserExtension)
						dhtmlObject.parserExtension._parseExtension(
								node.childNodes[i],
					...
					var zcall = dhtmlObject._parseXMLTree(dhtmlObject,
							node.childNodes[i], cId, 1);
					if (zcall != "")
						nodeAskingCall = zcall;
				} else if ((node.childNodes[i].nodeType == 1)
						&& (node.childNodes[i].tagName == "userdata")) {
					var name = node.childNodes[i].getAttribute("name");
					if ((name) && (node.childNodes[i].childNodes[0])) {
						dhtmlObject.setUserData(parentId, name,
								node.childNodes[i].childNodes[0].data);
					}
					;
				}
				;
		...
		dhtmlXTreeObject.prototype._redrawFrom = function(dhtmlObject,
				itemObject) {
			if (!itemObject) {
		...
		dhtmlXTreeObject.prototype._correctPlus = function(itemObject) {
			...
			if ((this.XMLsource) && (!itemObject.XMLload)) {
				...
				itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[2].childNodes[0].src = this.imPath
						+ itemObject.images[2];
			} else
				try {
			...
			if (!itemObject.treeNod.treeLinesOn)
				itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[0].childNodes[0].src = this.imPath
						+ workArray[3];
			else {
		...
		dhtmlXTreeObject.prototype._getCountStatus = function(itemId,
				itemObject) {
			try {
		...
		dhtmlXTreeObject.prototype._HideShow = function(itemObject, mode) {
			...
			if (Count > 1) {
				if (((Nodes[1].style.display != "none") || (mode == 1))
						&& (mode != 2))
					nodestyle = "none";
				else
					nodestyle = "";
		...
		dhtmlXTreeObject.prototype.onRowClick2 = function() {
			if (this.parentObject.treeNod.dblclickFuncHandler)
				if (!this.parentObject.treeNod
						.dblclickFuncHandler(this.parentObject.id))
					return 0;
			if ((this.parentObject.closeble)
					&& (this.parentObject.closeble != "0"))
		...
		dhtmlXTreeObject.prototype.onRowClick = function() {
			if (this.parentObject.treeNod.openFuncHandler)
				if (!this.parentObject.treeNod.openFuncHandler(
						this.parentObject.id, this.parentObject.treeNod
								._getOpenState(this.parentObject)))
					return 0;
			if ((this.parentObject.closeble)
					&& (this.parentObject.closeble != "0"))
				this.parentObject.treeNod._HideShow(this.parentObject);
		...
		dhtmlXTreeObject.prototype.onRowSelect = function(e, htmlObject, mode) {
			...
			if (!mode) {
				if (htmlObject.parentObject.actionHandler)
					htmlObject.parentObject
							.actionHandler(htmlObject.parentObject.id);
			}
		};
		...
		dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject,
				mode) {
			...
			td2.style.width = "100%";
			itemObject.span.appendChild(document
					.createTextNode(itemObject.label));
		...
		dhtmlXTreeObject.prototype.getSelectedItemId = function() {
			if (this.lastSelected)
				if (this
						._globalIdStorageFind(this.lastSelected.parentObject.id))
		...
		dhtmlXTreeObject.prototype.setItemColor = function(itemId,
		...
		dhtmlXTreeObject.prototype._moveNode = function(itemObject,
				targetObject) {
			if (this.dragFunc)
		...
		dhtmlXTreeObject.prototype._checkParenNodes = function(itemId,
				htmlObject, shtmlObject) {
			if (shtmlObject) {
		...
		dhtmlXTreeObject.prototype._registerBranch = function(itemObject,
				oldTree) {
		...
		dhtmlXTreeObject.prototype.setStdImages = function(image1, image2,
				image3) {
		...
		dhtmlXTreeObject.prototype.setImageArrays = function(arrayName, image1,
		...
		dhtmlXTreeObject.prototype.openItem = function(itemId) {
			...
			if ((temp.parentObject)
					&& (this._getOpenState(temp.parentObject) < 0))
		...
		dhtmlXTreeObject.prototype.setItemImage2 = function(itemId, image1,
				image2, image3) {
			var temp = this._globalIdStorageFind(itemId);
			if (!temp)
				return 0;
		...
		dhtmlXTreeObject.prototype.setItemImage = function(itemId, image1,
				image2) {
			var temp = this._globalIdStorageFind(itemId);
			if (!temp)
				return 0;
		...
		dhtmlXTreeObject.prototype.selectItem = function(itemId, mode) {
			...
			if (mode)
				this
						.onRowSelect(
								0,
								temp.htmlNode.childNodes[0].childNodes[0].childNodes[3],
								false);
			else
				this
						.onRowSelect(
		...
		dhtmlXTreeObject.prototype._deleteNode = function(itemId, htmlObject,
				skip) {
		...
		dhtmlXTreeObject.prototype._getAllChecked = function(htmlNode, list,
				mode) {
			if (!htmlNode)
		...
		dhtmlXTreeObject.prototype._deleteItem = function(itemId, selectParent,
				skip) {
		...
		dhtmlXTreeObject.prototype.insertNewNext = function(parentItemId,
				itemId, itemName, itemActionHandler, image1, image2, image3,
				optionStr, childs) {
			var sNode = this._globalIdStorageFind(parentItemId);
			if (!sNode)
				return (0);
			this._attachChildNode(0, itemId, itemName, itemActionHandler,
					image1, image2, image3, optionStr, childs, sNode);
		};
		...
		dhtmlXTreeObject.prototype.getChildItemIdByIndex = function(itemId,
				index) {
			var sNode = this._globalIdStorageFind(itemId);
			if (!sNode)
				return (0);
		...
		dhtmlXTreeObject.prototype._drag = function(sourceHtmlObject,
				dhtmlObject, targetHtmlObject) {
		...
		dhtmlXTreeObject.prototype._dragIn = function(htmlObject, shtmlObject) {
			...
			if ((!this._checkParenNodes(shtmlObject.parentObject.id,
